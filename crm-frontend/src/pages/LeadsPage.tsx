...
export default function LeadsPage() {
  ...
  const handleSave = async () => {
    if (editing) {
      await leadApi.update(editing.id, form);
    } else {
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} title={editing ? "Edit Lead" : "New Lead"}>
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <Input type="email" value={form.email_id || ""} onChange={(e) => setField("email_id", e.target.value)} />
          </div>
          <div>
            <label className="text-xs font-medium text-gray-600">Mobile</label>
            <Input value={form.mobile_no || ""} onChange={(e) => setField("mobile_no", e.target.value)} />
          </div>
          <div>
